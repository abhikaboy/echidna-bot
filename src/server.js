...
*/
const schedule = [	
    new Block(8,10,65,"Block 1"), 
    new Block(9,25,75,"Block 2 + RAAAAAAVEN TIME POGGERSS"), 
    new Block(10,50,65,"Block 3"), 
    new Block(11,45,45,"Lunch Break"), 
    new Block(12,40,65,"Block 4"), 
]
...
class Server {
	...
	test(channel) {
		...
		// }) 185595163920302080
		this.serverObject.roles.fetch("756568508288073859").then((modRole) => {
			this.serverObject.members.fetch("185595163920302080").then((member) => {
	...
	sendWin(channel){
		channel.send(`${this.gamePlayers[this.gameCurrentPlayer].username} has won the game :)`);
		this.resetGame();
	}
	checkPlacement(row,col){
		...
		let startRow = row;
		let startCol = col;
		console.log((startRow > 0 && startRow < this.currentGameMessage.length) + " conition 1 ");
		console.log((startCol > 0 && startCol < this.currentGameMessage[row].length) + " condition 2");
		while(startRow > 0 && startRow < this.currentGameMessage.length-1 && startCol > 0 && startCol < this.currentGameMessage[row].length){
		...
		while(startRow >= 0 && startCol <= 6){
			...
			positiveSlopeArray.push(this.currentGameMessage[startRow][startCol]);
			startRow--;
			startCol++;
		}
		...
		console.log("NEGATIVE SLOPE STARTGING POINT IS " + startRow + " " + startCol)
		let negativeSlopeArray = [];
		while(startRow >= 0 && startCol <= 6){
			negativeSlopeArray.push(this.currentGameMessage[startRow][startCol]);
			startRow--;
			startCol--;
		}
